...
bool GetSpoofStatus(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL10) /*&& (blurValue >= 1700 && (stdValue >= 40 && stdValue <= 65) && (meanValue > 85 && meanValue <150))*/)
	{
		skipFrame++;
		lowPwm++;
	...
	if ((skipFrame > 2) && (lowPwm > pwmchangecount))
	{
        ...
        if (((high_meanvalue > mid_meanvalue) && (mid_meanvalue > low_meanvalue)) && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 40) && ((irPropety[irPropety.size() - 1].blur > 500) && ((irPropety[irPropety.size() - 1].blur < 7000))))
		{
			FILE* fptr = NULL;
...
*/
			...
			retValue = MEngin_GetImageSimilarityScore(g_ipl_info[0].Final_Frame, Prev_Frame_Preview, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, 20, m_gSimilarityScore[m_NoOfFrameCount % 3]);
			if (retValue != 0)
			{
				/*_snprintf_s(szOutputString, sizeof(szOutputString), sizeof(szOutputString),
