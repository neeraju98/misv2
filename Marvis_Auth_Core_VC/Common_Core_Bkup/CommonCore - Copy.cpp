	...
	int iris_center_x, int iris_center_y, int pupil_center_r, unsigned char* outdata, int *out_black_count)
{
	...
	int StartX = iris_center_x - pEyeInfo.pupil_diameter / 2;
	int StartY = iris_center_y - pEyeInfo.pupil_diameter / 2;
	...
	for (int i = StartY; i < height + StartY; i++)
	{
		for (int j = StartX; j < StartX + width; j++)
		{
			...
			if ((circleDist < (pEyeInfo.pupil_diameter / 2)) && (i > pupil_y) && (i < pupil_y + pupil_height))
			{
	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			else
			{
				...
				/*unsigned char filePathname[2048] = { '\0' };
				sprintf((char*)&filePathname[0], "%d_FinalFrame_%d.raw", counter,filecounter++);
				WriteAllText1((const char*)filePathname, (char*)g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH*g_ipl_info[0].FINAL_FRAME_HEIGHT); 
*/
				...
				{
					/*unsigned char filePathname[2048] = { '\0' };
					sprintf((char*)&filePathname[0], "%d_FinalFrame_%d.raw", filecounter,counter);
					WriteAllText1((const char*)filePathname, (char*)g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH*g_ipl_info[0].FINAL_FRAME_HEIGHT);*/
				...
				if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0)){
					if (IRLed_Status)
					{
						...
						retValue = CommSetIRLed(COMM_DUTY_CYCLE_OFF);
				...
				//if ((mean >= 150 && mean  < 180) && ((blur_value > 7000) || (blur_value > 6800)))
				//{
				//	g_ipl_info[0].pQtyInfo.Quality = 0;
				//}
				//else if ((mean >= 180 && mean <= 200) && ((blur_value > 6000) || (blur_value > 5800)))
				//{
				//	g_ipl_info[0].pQtyInfo.Quality = 0;
				//}
				//else if ((mean > 200 && mean <= 210) && ((blur_value > 5000) || (blur_value > 4800)))
				//{
				//	g_ipl_info[0].pQtyInfo.Quality = 0;
				//}
				//else if ((mean > 210 /*&& mean <= 210*/) && ((blur_value > 4500) || (blur_value > 4300)))
				//{
				//	g_ipl_info[0].pQtyInfo.Quality = 0;
				//}
				//
				g_ipl_info[0].pQtyInfo.percent_occlusion = m_gSimilarityScore[m_NoOfFrameCount % 2];
				g_ipl_info[0].pQtyInfo.iris_mean = mean;
				g_ipl_info[0].pQtyInfo.pupil_mean = pEyeInfo.pupil_mean;
				g_ipl_info[0].pQtyInfo.pupil_blur = blur_value;
				g_ipl_info[0].pQtyInfo.iris_diameter = 0;// white_count;
				g_ipl_info[0].pQtyInfo.pupil_diameter = 0;// black_count;
				...
				//else
				{
					//g_ipl_info[0].pBitmapImage.
					//WriteAllText1((const char*)"final.raw", (char*)g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH*g_ipl_info[0].FINAL_FRAME_HEIGHT);
					//retValue = iplConvertRawToBmp(g_ipl_info[0].pBitmapImage.m_BitmapImage, g_ipl_info[0].pBitmapImage.m_BitmapImage, g_ipl_info[0].FINAL_FRAME_HEIGHT, g_ipl_info[0].FINAL_FRAME_WIDTH, 0);
					//IsPreviewHide = false;
				}
