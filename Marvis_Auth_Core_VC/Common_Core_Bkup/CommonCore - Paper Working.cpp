...
int  average_textureff = 0, average_textureiris = 0, average_high_mean = 0, average_low_mean = 0;
int mean_count = 0, mean_count_1=0;
#pragma region Variable Declaration

int maximum = 0; double textureFeature, textureFeaturecircledata, pcount = 0.0;
int minimum = 0;
int First = 0;
...
float  Adjacent_pixel_count(unsigned char* RawData, int width, int height)
{
	...
	int iris_center_x, int iris_center_y, int pupil_center_r, unsigned char* outdata, int *out_black_count)
{
	...
	textureFeaturecircledata = CalculateTextureFeature(outdata, outlen, outlen);//g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
...
/*we can also do averages for mean in 17 level and lower level for more accuracy     */
bool GetSpoofStatus(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	lap_c = Adjacent_pixel_count(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
	...
	int outlen = g_ipl_info[0].pQtyInfo.R * 2;
	unsigned char* out_data = (unsigned char*)malloc(outlen*outlen);
	//int black_count = 0;
	...
	stcimgeproperty.texture_circle = textureFeaturecircledata;
	stcimgeproperty.pixelcount = pcount;
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17) && (!IsSpoofStatus))
	{
		if (max_high_texture - min_high_texture > 10 || (max_high_meanvalue - min_high_meanvalue>20))
		{
			...
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%d,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "timeout", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].sharpness, irPropety[i].laplacian_count, max_lap, min_lap);
				//fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "timeout", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);

			
			////////////////////////////////////////////////////to stop capture immediately if fake case detected
			//m_hCaptureCompleteThreadHandle = ::CreateThread(NULL, 0, &StopCapThread, 0, 0, NULL);
			//IsGetQtyInCap = false;
			//DeviceErrorCode = MARVIS_AUTH_E_NULLPARAM;
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL11))
	{
		...
		else if (((irPropety[0].texture - irPropety[irPropety.size() - 1].texture) > 30) && (max_high_meanvalue > max_low_meanvalue)/* && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 20)*/ && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) /*&& pcount>10*/ && (max_lap - min_lap)< 15)
			//else if ((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>30 && ((high_meanvalue > low_meanvalue))) //&& ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 20)) || (((high_meanvalue > low_meanvalue)) && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 25)))
		{
			if (/*(((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && (irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) < 2) &&*/ (average_textureff > 200))
			{
				lowpwmhighmean = 0;
				updown = true;
				...
				for (int i = 0; i < irPropety.size(); i++)
				{
					fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%d,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].sharpness, irPropety[i].laplacian_count, max_lap, min_lap);
					//fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle, irPropety[i].pixelcount);
				}
				fprintf(fptr1, "\n");
				fclose(fptr1);
			}
	...
	{
		if ((max_high_meanvalue > max_low_meanvalue) && ((average_high_mean/*irPropety[0].mean */ - average_low_mean /*irPropety[irPropety.size() - 1].mean*/) >= 10) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && (max_lap - min_lap)< 15)
		//if (((high_meanvalue > low_meanvalue)) && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 35))
		{
			if ((average_textureff > 200 ))
			{
				...
				{
					fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%d,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].sharpness, irPropety[i].laplacian_count, max_lap, min_lap);
					//fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle);
				}
				fprintf(fptr1, "\n");
				fclose(fptr1);
			}
	...
	if (lowpwmhighmean >= 6)
	{
		...
		for (int i = 0; i < irPropety.size(); i++)
		{
			fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%d,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Not Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].sharpness, irPropety[i].laplacian_count, max_lap, min_lap);
			//fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Not Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle);
		}
		fprintf(fptr1, "\n");
		fclose(fptr1);
...
bool GetSpoofStatus1(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	double stdValue = 0.0;
	textureFeature = 0;
	int Innercount = 0, OuterCount = 0;float lap_c = 0.0;
	int sum_textureff = 0, sum_textureiris = 0;
	/*int arr1[4] = { 2, 3, 4, 5 };
	int arr2[3] = { 17, 13, 10 };
	pwmchangecount = arr1[rand() % 4];*/
	...
	/*stcimgeproperty.avg1 = average_textureff;
	stcimgeproperty.avg2 = average_textureiris;*/
	irPropety.push_back(stcimgeproperty);
	...
	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL14) /*&& (blurValue >= 1700 && (stdValue >= 40 && stdValue <= 65) && (meanValue > 85 && meanValue <150))*/)
	{
		skipFrame++;
		if (max_low_meanvalue < irPropety[irPropety.size() - 1].mean)
			max_low_meanvalue = irPropety[irPropety.size() - 1].mean;
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL14))
	{
		...
		else if (((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>5) && (max_high_meanvalue > max_low_meanvalue)/* && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 20)*/ && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean /*average_low_mean*/ / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && (max_lap-min_lap)< 10)
		{
			if (/*(((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && (irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) < 2) && */(average_textureff > 200 ))
			{
				lowpwmhighmean = 0;
	...
	if (lowpwmhighmean >= 1 && lowpwmhighmean < 6)
	{
		if ((max_high_meanvalue > max_low_meanvalue) && ((average_high_mean/*irPropety[0].mean*/ - average_low_mean /*irPropety[irPropety.size() - 1].mean*/) >= 20) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean/*average_low_mean*/ / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && (max_lap - min_lap)< 10)
		{
			if (/*(((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && (irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) < 2) &&*/ (average_textureff > 200 ))
			{
				lowpwmhighmean = 0;
...
//	//}
//
//	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL9) /*&& (blurValue >= 1700 && (stdValue >= 40 && stdValue <= 65) && (meanValue > 85 && meanValue <150))*/)
//	{
//		skipFrame++;
//		lowPwm++;
...
//	}
//	else*/ 
//	if ((skipFrame > 1) && (highPwm >=3) && (!IsSpoofStatus))
//	{
...
//		else
//		{
//			retvalue = CommSetIRLed(COMM_DUTY_CYCLE_LEVEL9);
//			prev_pwmValue = COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL9;
//			//pwmchangecount = arr1[rand() % 4];
//			::Sleep(200);
//			skipFrame = 0;
//			highPwm = 0;
	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			*/
			...
			{
				if (nMinQty == 55)
					ISComplete = GetSpoofStatus1(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &mean, &stddev, &blur);
...
int MARVIS_Auth_StartCapture(int in_quality, int TimeOut, PREVIEW_CALLBACK p_PREVIEW_CALLBACK, CAPTURE_COMPLETE_CALLBACK p_CAPTURE_COMPLETE_CALLBACK, char* empid)
{
	...
	try
	{
		...
		average_high_mean = 0, average_low_mean = 0;
		max_low_meanvalue = 0;
