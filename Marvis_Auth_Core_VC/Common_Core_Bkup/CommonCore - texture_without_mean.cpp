...
bool GetSpoofStatus(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	if ((skipFrame > 2) && (lowPwm > pwmchangecount))
	{
		...
		if ((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>30 && ((high_meanvalue > mid_meanvalue) && (mid_meanvalue > low_meanvalue))/* && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 40)*/)
		{
			FILE* fptr = NULL;
			...
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);
		}
