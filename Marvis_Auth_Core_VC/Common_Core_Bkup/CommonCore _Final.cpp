	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			*/
			...
			if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0 || no_quality) && !updown && lowpwmhighmean < 6)
			{
				if (nMinQty == 55)//yellow 
					ISComplete = GetSpoofStatus1();


				else
					ISComplete = GetSpoofStatus();
			}
			else
			{
				skipFrame = 0;
