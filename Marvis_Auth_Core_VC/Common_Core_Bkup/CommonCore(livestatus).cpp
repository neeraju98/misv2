...
double endtime = 0;
bool updown = false;
int high_max_meanvalue = 0;
int high_min_meanvalue = 0;

int mid_meanvalue = 0;

int low_max_meanvalue = 0;
int low_min_meanvalue = 0;


int quality_fail_count = 0;
int texture_FF = 0;
int texture_IRIS = 0;

//// ssp new logic
#define PWM_SEQUENCE_COUNT  3
typedef struct
{
	uint8_t Spoof_Enable;
	uint8_t Qty_Thresold;
	uint16_t LScore_Dry;
	uint16_t LScore_Normal;
	uint16_t LScore_LiveNormal;
	uint16_t Lapals_Score_Lower;
	uint16_t Lapals_Score_Higher;
	uint8_t LowStdDev_Count;
	uint8_t HighStdDev_Count;
	uint8_t PwmSetCount;
	uint8_t WhiteCount;
	uint8_t PWMHigherCount;
	uint8_t PWMMiddleCount;
	uint8_t PWMLowerCount;
	uint8_t SimilarityScore;
	uint8_t StdDevGreedCount;
	uint16_t OutDoorHistLowerThresold;
	uint16_t OutDoorHistLowerUpper;
	uint8_t LowerStdDevThresold;
	uint8_t LowerStdDevThresoldCount;
	uint8_t ImproperFingerCount;
	uint16_t OutHistUpper;
	uint16_t OutHistMiddle_1;
	uint16_t OutHistMiddle_2;
	uint16_t OutHistMiddle_3;
}t_SpoofThresold_Struct;
...
struct PwmStruct
{
	...
	uint8_t pwm;
	uint8_t stdGridCnt;
	uint16_t OutHistLower;
	uint16_t OutHistMiddle;
	uint16_t OutHistUpper;
}PwmListInfo[PWM_SEQUENCE_COUNT];

static t_SpoofThresold_Struct SpoofParamThresold = { 1, 25, 95, 90, 99, 1500, 2500, 0, 0, 70, 30, 75, 50, 25, 95, 15, 10000, 25000, 20, 2, 1, 18000, 11000, 9000, 1500 };
uint8_t Mean = 0;
uint8_t MeanDis = 0;
uint16_t Laplase_Score = 0;
static uint8_t Livestatus = 1; /*0= Live, 1 = Fake*/
float S_Score = 0.00;
uint8_t StdDev = 0;
uint8_t StdDevDiff = 0;
//static uint8_t NonSimilarityFrameCnt = 0;
static uint8_t NormalMean = 0;
static uint8_t NormalStdDev = 0;
static uint32_t OutHistLower = 0;
static uint32_t OutHistMiddle = 0;
static uint32_t OutHistUpper = 0;
static uint8_t LowStdGridCount = 0;
uint8_t Whitecnt = 0;
static uint8_t PwmRestCycleCntSpoof = 0;


uint8_t DerivePwm[PWM_SEQUENCE_COUNT + 1] = { 17, 12, 8, 17 };
uint8_t pwmList[] = { 0, 0, 0 };
uint8_t pwmListIndex = 0;
uint8_t cur_PwmIndex = 0;

#define HOR_PER     50
#define VER_PER     70

uint8_t StartCapture_flag = 0;
bool Is_YellowOrpurple = true; // purple
////
...
void WriteFullAndFinalFrame(const char *in_full_frame_name, char *in_full_data, int in_full_datalength,const char *in_final_frame_name, char *in_final_data, int in_final_datalength)
{
	int WriteFile = 1;

	if (WriteFile == 0)
	{
		char *bmpimgdata = (char*)malloc((g_ipl_info[0].FULL_FRAME_HEIGHT*g_ipl_info[0].FULL_FRAME_WIDTH) + 1078 + 1);
		memset(bmpimgdata, 0, (g_ipl_info[0].FULL_FRAME_HEIGHT*g_ipl_info[0].FULL_FRAME_WIDTH) + 1078 + 1);

		FILE *file = NULL;
		errno_t err = 0;
		err = fopen_s(&file, in_full_frame_name, "wb");
		if (err == 0)
		{
			iplConvertRawToBmp((unsigned char*)in_full_data, (unsigned char*)bmpimgdata, g_ipl_info[0].FULL_FRAME_HEIGHT, g_ipl_info[0].FULL_FRAME_WIDTH, 0);
			fwrite(bmpimgdata, 1, in_full_datalength+1078, file);
			fclose(file);
		}
		...
		FILE *file1 = NULL;
		errno_t err1 = 0;
		err1 = fopen_s(&file1, in_final_frame_name, "wb");
		if (err1 == 0)
		{
			iplConvertRawToBmp((unsigned char*)in_final_data, (unsigned char*)bmpimgdata, g_ipl_info[0].FINAL_FRAME_HEIGHT, g_ipl_info[0].FINAL_FRAME_WIDTH, 0);
			fwrite(bmpimgdata, 1, in_final_datalength + 1078, file1);
			fclose(file1);
		}
...
int  Circledata(unsigned char *in_rawImg, int ImgWidth, int ImgHeight,
	int iris_center_x, int iris_center_y, int pupil_center_r, unsigned char* outdata, double *out_black_count)
{
	/*int white_pixel_count = 0;
	int black_pixel_count = 0;*/
	double mean = 0;
	int std_dev = 0;
	pupil_center_r = pEyeInfo.iris_diameter / 2;
	/*int StartX = iris_center_x - pEyeInfo.iris_diameter / 2;
	int StartY = iris_center_y - pEyeInfo.iris_diameter / 2;*/

	int StartX = iris_center_x - pupil_center_r;
	int StartY = iris_center_y - pupil_center_r;

	int width = pEyeInfo.iris_diameter ;
	int height = pEyeInfo.iris_diameter ;
	...
	for (int i = StartY; i < height + StartY; i++)
	{
		for (int j = StartX; j < StartX + width; j++)
		{
			...
			//if ((circleDist < (pupil_center_r)) && (i > pupil_y) && (i < pupil_y + pupil_height))
			{
	...
	for (int i = 0; i < counter; ++i) {
		double diff = outdata[i] - mean;
		sumSquaredDiff += diff * diff;
	}
	double variance = sumSquaredDiff / counter;
	*out_black_count = sqrt(variance);*/
...
bool GetSpoofStatus(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	int mean = Circledata(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, g_ipl_info[0].pQtyInfo.X, g_ipl_info[0].pQtyInfo.Y, g_ipl_info[0].pQtyInfo.R, out_data, &std_dev);
	...
	stcimgeproperty.mean = meanValue;// mean;
	stcimgeproperty.sd = stdValue;// std_dev;
	...
	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17) /*&& (blurValue >= 3000 && (stdValue >= 55 && stdValue <= 75) && (meanValue > 180 && meanValue <220))*/)
	{
		skipFrame++;
		highPwm++;
		if (high_max_meanvalue < irPropety[irPropety.size() - 1].mean)
			high_max_meanvalue = irPropety[irPropety.size() - 1].mean;

		if ((high_min_meanvalue > irPropety[irPropety.size() - 1].mean) || (high_min_meanvalue == 0))
			high_min_meanvalue = irPropety[irPropety.size() - 1].mean;
	}
	...
	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL10) || (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL11))  /*&& (blurValue >= 1700 && (stdValue >= 40 && stdValue <= 65) && (meanValue > 85 && meanValue <150))*/
	{
		skipFrame++;
		lowPwm++;
		if (low_max_meanvalue < irPropety[irPropety.size() - 1].mean)
			low_max_meanvalue = irPropety[irPropety.size() - 1].mean;
		
		if ((low_min_meanvalue > irPropety[irPropety.size() - 1].mean) || (low_min_meanvalue == 0))
			low_min_meanvalue = irPropety[irPropety.size() - 1].mean;
		lowblurvalue = irPropety[irPropety.size() - 1].blur;
	}
	...
	/*if ((skipFrame > 2) && (highPwm > pwmchangecount) && (!IsSpoofStatus))
	{
		retvalue = CommSetIRLed(COMM_DUTY_CYCLE_LEVEL13);
		prev_pwmValue = COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL13;
		pwmchangecount = arr1[rand() % 4];
		::Sleep(200);
		skipFrame = 0;
		highPwm = 0;
	...
	{
		//high to low
		//Find Minimum mean and Maximum mean, diff <10 =ok down
		//reset all data, skipFrame = 0

		if ((high_max_meanvalue - high_min_meanvalue) > 10)
		{
			skipFrame = 0;
		}
		else
	...
	if ((skipFrame > 2) && (lowPwm > pwmchangecount))
	{
		//low to high
		//Find Minimum mean and Maximum mean, diff <10 =ok up
		//reset all data, skipFrame = 0
		if ((low_max_meanvalue - low_min_meanvalue) > 10)
		{
			skipFrame = 0;
		}
		else
		{
			...
			if(((high_max_meanvalue > low_max_meanvalue) /*&& (mid_meanvalue > low_meanvalue)*/) && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 40) && ((irPropety[irPropety.size() - 1].blur > 500) && ((irPropety[irPropety.size() - 1].blur < 7000))))
				//if (((high_meanvalue > low_meanvalue) /*&& (mid_meanvalue > low_meanvalue)*/) && ((high_meanvalue - low_meanvalue) >= 40) && ((lowblurvalue > 500) && ((lowblurvalue < 7000))))
			{
				FILE* fptr = NULL;
				fopen_s(&fptr, "Mean_StdDev.csv", "ab");
				fprintf(fptr, "%s,%d,%d,%d,%d,%d,%s\n", emp_id, irPropety[0].mean, irPropety[irPropety.size() - 1].mean, irPropety[0].sd, irPropety[irPropety.size() - 1].sd, irPropety[irPropety.size() - 1].blur, "Captured");
				fclose(fptr);
				lowpwmhighmean = 0;
				updown = true;
				retvalue = CommSetIRLed(COMM_DUTY_CYCLE_LEVEL17);
				...
				{
					fprintf(fptr1, "%s,%d,%d,%d,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur);
				}
				fprintf(fptr1, "\n");
				fclose(fptr1);
			}
	...
	if (lowpwmhighmean >= 6)
	{
		FILE* fptr = NULL;
		fopen_s(&fptr, "Mean_StdDev.csv", "ab");
		fprintf(fptr, "%s,%d,%d,%d,%d,%d,%s\n", emp_id, irPropety[0].mean, irPropety[irPropety.size() - 1].mean, irPropety[0].sd, irPropety[irPropety.size() - 1].sd, irPropety[irPropety.size() - 1].blur, "Not Captured");
		fclose(fptr);
...
bool LogSpoofParameters(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	bool IsSpoofStatus = false;
	double meanValue = 0.0;
	double blurValue = 0.0;
	...
	unsigned char* out_data = (unsigned char*)malloc(g_ipl_info[0].FINAL_FRAME_WIDTH* g_ipl_info[0].FINAL_FRAME_HEIGHT);
	int mean = Circledata(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, g_ipl_info[0].pQtyInfo.X, g_ipl_info[0].pQtyInfo.Y, g_ipl_info[0].pQtyInfo.R, out_data, &std_dev);
	double textureFeature = CalculateTextureFeature(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
	int retvalue = Getblur(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &meanValue, &blurValue);
	retvalue = GetMeanAndStddev(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &meanValue, &stdValue);
	...
	stcimgeproperty.cir_mean = mean;
	stcimgeproperty.cir_sd = std_dev;
	stcimgeproperty.texture_value = textureFeature;
	irPropety.push_back(stcimgeproperty);


	FILE* fptr2 = NULL;
	fopen_s(&fptr2, "ImageParameters.csv", "ab");
	fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d\n", emp_id, stcimgeproperty.irValue, stcimgeproperty.mean, stcimgeproperty.sd, stcimgeproperty.blur, stcimgeproperty.cir_mean, stcimgeproperty.cir_sd, stcimgeproperty.texture_value);
	fclose(fptr2);
	...
	if (skipFrame % 10 == 0)
	{
		...
		unsigned char fullImgname[2048] = { '\0' };
		sprintf((char*)&fullImgname[0], "fullFinalCombo\\%s_FullFrame_%d_PWM_%d.bmp", emp_id, filecounter, prev_pwmValue);

		unsigned char finalImgname[2048] = { '\0' };
		sprintf((char*)&finalImgname[0], "fullFinalCombo\\%s_FinalFrame_%d_PWM_%d.bmp", emp_id, filecounter, prev_pwmValue);
		...
		WriteFullAndFinalFrame((const char*)fullImgname, (char*)g_ipl_info[0].RawImageBuffer, g_ipl_info[0].FULL_FRAME_WIDTH*g_ipl_info[0].FULL_FRAME_HEIGHT,
			(const char*)finalImgname, (char*)g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH*g_ipl_info[0].FINAL_FRAME_HEIGHT);
		...
		prev_pwmValue = PWM;
		retvalue = CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)prev_pwmValue);
		::Sleep(200);
	}
...
int is_sorted(struct PwmStruct* array, int length) {
	for (int i = 0; i < length - 1; i++) {
		if ((array[i].std > array[i + 1].std) || (abs(array[i].std - array[i + 1].std) <= 5)) {
...
// Function to check if an array is sorted
int is_sortedHistSum(struct PwmStruct* array, int length) {
	for (int i = 0; i < length - 1; i++) {
		//if (!((abs(array[i].mean - array[i + 1].mean) > 10) && (array[i].OutHistLower < array[i + 1].OutHistLower)))
		if ((array[i].mean > array[i + 1].mean) || (abs(array[i].mean - array[i + 1].mean) <= 10)) {
			return 0; // Return 0 (false) if the array is not sorted
		}
	}
	return 1; // Return 1 (true) if the array is sorted
...
// Function to check if an array is sorted
int is_sortedTemperDevice(struct PwmStruct* array, int length) {
	for (int i = 0; i < length - 1; i++) {
		if (!((abs(array[i].mean - array[i + 1].mean) < 5) && abs((array[i].std - array[i + 1].std) <= 5)
			&& (abs(array[i].stdGridCnt - array[i + 1].stdGridCnt) <= 5))) {
			return 0; // Return 0 (false) if the array is not sorted
		}
	}
	return 1; // Return 1 (true) if the array is sorted
...
int is_outdoorDevice(struct PwmStruct* array, int length) {


	for (int i = 0; i < length; i++) {
		if ((array[i].OutHistLower < SpoofParamThresold.OutDoorHistLowerThresold) ||
			array[i].OutHistMiddle > SpoofParamThresold.OutDoorHistLowerThresold ||
			(abs(array[i].OutHistLower + array[i].OutHistUpper) < SpoofParamThresold.OutDoorHistLowerUpper))
		{
			return 0; // false
		}
	}
...
uint16_t computeLaplaseScore(uint8_t * src_raw, uint16_t src_width, uint16_t src_height)
{

	int newWidth = (HOR_PER * src_width) / 100;
	int newHeight = (VER_PER * src_height) / 100;

	int xw1 = (src_width / 2) - (newWidth / 2);
	int xw2 = xw1 + newWidth;
	int yh1 = (src_height / 2) - (newHeight / 2);
	int yh2 = yh1 + newHeight;

	float mean = 0.0, variance = 0.0;

	// Calculate the Laplacian of the image
	for (int y = yh1; y < yh2; y++) {
		for (int x = xw1; x < xw2; x++) {

			int laplacianValue = -4 * src_raw[(y * src_width) + x] +
				src_raw[((y - 1) * src_width) + x] + src_raw[((y + 1) * src_width) + x] +
				src_raw[(y * src_width) + (x - 1)] + src_raw[(y * src_width) + (x + 1)];

			if (laplacianValue > 255) laplacianValue = 255;
			if (laplacianValue < 0) laplacianValue = 0;
	...
	for (int y = yh1; y < yh2; y++) {
		for (int x = xw1; x < xw2; x++) {
			...
			float diff = laplacianValue - mean;
			variance += diff * diff;
		}
	}
	variance /= (newWidth * newHeight);

	return (uint16_t)variance;
}

int MEngin_GetHistData(const unsigned char *rawData, int width, int height)
{
	if (rawData == NULL)
	{
		return -10;
	...
	for (size_t row = 0; row < height; row++)
	{
		...
		{
			int val = (255 - rawData[(width * row) + col]);
			hist[val] ++;
		}
	}
...
void drawFilledRectangle(unsigned char* imageData, int imageWidth, int x1, int y1, int x2, int y2, int imageHeight, unsigned char*outimage) {
	// Clamp rectangle coordinates within image bounds
	if (x1 < 0) x1 = 0;
	if (y1 < 0) y1 = 0;
	...
	int i = 0;
	// Iterate over the rectangle region and set pixel values to black (0)
	for (int y = y1; y < y2; ++y) {
		for (int x = x1; x < x2; ++x) {
			int index = (y * imageWidth + x);
			outimage[i++] = imageData[index];
			//imageData[index] = 0; // Set pixel to black			
		}
	}
}

double Calculatepixels(unsigned char* imageData, int width, int height) {
...
double GetTextureData_IRIS_FF(unsigned char* RawData, int width, int height, int Centre_X, int Centre_Y, int Circle_Radius, 
	int *Innercount, int *OuterCount,int *out_IrisMean, int *out_IrisSd)
{
	int Inner = 0;
	int Outer = 0;
	...
	if (Centre_X != 0 &&
		...
		Circle_Radius != 0)
	{
		int croppedWidth = 2 * Circle_Radius;
		int croppedHeight = 2 * Circle_Radius;
		unsigned char* croppedImage = (unsigned char*)malloc(croppedWidth * croppedHeight);

		int rect1X = Centre_X - Circle_Radius;
		int rect1Y = Centre_Y - Circle_Radius;
		if (rect1Y < 0)
		{
			rect1Y = 0;
		}
		...
		drawFilledRectangle(RawData, width, rect1X, rect1Y, rect1X + croppedWidth, rect1Y + croppedHeight, height, croppedImage);

		double meanValue = 0.0;
		double stdValue = 0.0;
		int retvalue = GetMeanAndStddev(croppedImage, croppedWidth, croppedHeight, &meanValue, &stdValue);
		*out_IrisMean = (int)meanValue;
		*out_IrisSd = (int)stdValue;

		for (int y = 1; y < croppedHeight - 1; ++y)
		{
			for (int x = 1; x < croppedWidth - 1; ++x)
			{
				unsigned char center = croppedImage[y * croppedWidth + x];
				unsigned char neighbors[8] = {
					croppedImage[(y - 1) * croppedWidth + (x - 1)], croppedImage[(y - 1) * croppedWidth + x], croppedImage[(y - 1) * croppedWidth + (x + 1)],
					croppedImage[y * croppedWidth + (x - 1)], croppedImage[y * croppedWidth + (x + 1)],
					croppedImage[(y + 1) * croppedWidth + (x - 1)], croppedImage[(y + 1) * croppedWidth + x], croppedImage[(y + 1) * croppedWidth + (x + 1)]
				};


				double sum = neighbors[0] + neighbors[1] + neighbors[2] + neighbors[3] + neighbors[4] + neighbors[5] + neighbors[6] + neighbors[7];
				double average = sum / 8;

				double diff = abs(average - center);
	...
	*Innercount = Inner;
	...
	for (int y = 1; y < height - 1; ++y)
	{
		for (int x = 1; x < width - 1; ++x)
		{
			unsigned char center = RawData[y * width + x];
			unsigned char neighbors[8] = {
				RawData[(y - 1) * width + (x - 1)], RawData[(y - 1) * width + x], RawData[(y - 1) * width + (x + 1)],
				RawData[y * width + (x - 1)], /*center*/ RawData[y * width + (x + 1)],
				RawData[(y + 1) * width + (x - 1)], RawData[(y + 1) * width + x], RawData[(y + 1) * width + (x + 1)]
			};
...
uint8_t ImageSSPCheck(unsigned char* in_inputImageData, int in_Width, int in_Height, uint8_t Quality, uint8_t StartCapture, 
	double* out_meanValue, double* out_stddev, double *out_Blur, int *out_IrisMean, bool Is_Purple)
{
	int retvalue = 0;
	double meanValue = 0.0;
	double blurValue = 0.0;
	double stdValue = 0.0;
	double textureFeaturefullFrame = 0.0;
	double textureFeatureIrisFrame = 0.0;
	int pcount = 0;
	imgProperty imgirproperty;
	Livestatus = 1;
	int IrisMean = 0;
	int IrisSd = 0;
	texture_FF = 0;
	texture_IRIS = 0;
	GetTextureData_IRIS_FF(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT,
		g_ipl_info[0].pQtyInfo.X, g_ipl_info[0].pQtyInfo.Y, pEyeInfo.iris_diameter / 2, &texture_IRIS, &texture_FF,&IrisMean,&IrisSd);

	pcount = Calculatepixels(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);

	retvalue = Getblur(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &meanValue, &blurValue);
	retvalue = GetMeanAndStddev(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &meanValue, &stdValue);
	StdDev = (uint8_t)stdValue;
	Mean = (uint8_t)meanValue;
	*out_meanValue = Mean;
	*out_Blur = blurValue;
	*out_IrisMean = IrisMean;
	int curr_pwm = prev_pwmValue;
	imgirproperty.irValue = curr_pwm;
	imgirproperty.mean = Mean;
	imgirproperty.blur = blurValue;
	imgirproperty.texture_value = texture_FF;
	irPropety.push_back(imgirproperty);
	...
	if (PWM == 2)
	{
		int diff = high_max_meanvalue -  low_max_meanvalue;
		if (diff < 10)  //can add iris mean
		{
			Livestatus = 12;
			goto END;
		}
	...
	if (PWM==2 && Mean > 120 && Mean < 185 && texture_FF < 10000 /*&& texture_IRIS < 1500*/ && blurValue >500 && blurValue < 7000 && g_ipl_info[0].pQtyInfo.iris_mean < 200/* && pcount<180000*/)
	{
		Livestatus = 5;
		ISComplete = true;
	}
	else if (PWM==2 && Mean > 190 && Mean < 230 && /*texture_FF < 10000 &&*/ texture_IRIS < 1500 && blurValue >500 && blurValue < 7000 && g_ipl_info[0].pQtyInfo.iris_mean < 200 /*&& pcount<180000*/) //Fake Capture Success texture_IRIS = 1200
	{
		Livestatus = 5;
		ISComplete = true;
	}
	else if (Mean > 185 && texture_FF > 10000 && PWM == 2)
	{
		Livestatus = 12;
	}
	else if (/*Mean > 185 && */skipFrame == 1 && PWM == 0)
	{
		high_max_meanvalue = irPropety[irPropety.size() - 1].mean;
		skipFrame = 0;
		PWM++;
		...
		CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)prev_pwmValue);
		::Sleep(200);
	}
	else if (/*Mean > 185 || */skipFrame == 1 && PWM == 1)
	{
		low_max_meanvalue = irPropety[irPropety.size() - 1].mean;
		skipFrame = 0;
		PWM++;
		prev_pwmValue = pwm_defaultValue;
		CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)prev_pwmValue);
		::Sleep(200);
	}
	else if ((irPropety.size() > 10 || PWM == 2) && texture_FF > 10000) // change in these condition
	{
		Livestatus = 12;
	}
	...
	END:
	FILE* fptr2 = NULL;
	fopen_s(&fptr2, "ImageSSPCheck.csv", "ab");
	fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", emp_id, curr_pwm, (uint8_t)Mean, (uint8_t)StdDev,
		(int)blurValue, Livestatus, pEyeInfo.iris_quality, texture_FF, texture_IRIS, IrisMean, pcount);

	//fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", emp_id, prev_pwmValue, (uint8_t)Mean, (uint8_t)StdDev, Laplase_Score, (uint8_t)LowStdGridCount, (uint16_t)OutHistLower, (uint16_t)OutHistMiddle, (uint16_t)OutHistUpper, Livestatus);
	fflush(fptr2);
	fclose(fptr2);
...
uint8_t ImageSSPCheck1(unsigned char* in_inputImageData, int in_Width, int in_Height, uint8_t Quality, uint8_t StartCapture, double* out_meanValue, double* out_stddev, double *out_Blur, bool Is_Purple)
{
	if (StartCapture)
	{
		/*StartCapture_flag = 0;*/
		...
		MeanDis = 0;
		NormalMean = 0;
		pwmList[0] = DerivePwm[0];
		pwmList[1] = DerivePwm[1];
		pwmList[2] = DerivePwm[2];
		pwmListIndex = 0;
		/*J1 = 0;
		J2 = 0;
		J3 = 0;
		shuffle(&J1, &J2, &J3, RndNump);
		*/
	}
	...
	*out_meanValue = Mean;
	if (StartCapture)
	{
		Laplase_Score = computeLaplaseScore(in_inputImageData, in_Width, in_Height);
		//if (Laplase_Score > SpoofParamThresold.Lapals_Score_Higher)
		if (Quality > 0)
		{
			//*PwmValue = pwmList[pwmListIndex];
			Livestatus = 2; //New Pwm Value Set and Start Capture flag reset
			prev_pwmValue = (COMM_DUTY_CYCLE_LEVEL)pwmList[pwmListIndex];
			CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)pwmList[pwmListIndex]);
			::Sleep(200);
			skipFrame1 = 2;
			StartCapture_flag = 0;
		}
	...
	{
		MEngin_GetHistData(in_inputImageData, in_Width, in_Height);

		if (pwmListIndex < PWM_SEQUENCE_COUNT)
		{
			uint8_t idx = 0;
			if (pwmList[pwmListIndex] == DerivePwm[0])
			{
				idx = 2;
			}
			else if (pwmList[pwmListIndex] == DerivePwm[1])
			{
				idx = 1;
			}
			else if (pwmList[pwmListIndex] == DerivePwm[2])
			{
				idx = 0;
			}

			PwmListInfo[idx].std			= (uint8_t)StdDev;
			PwmListInfo[idx].mean			= (uint8_t)Mean;
			PwmListInfo[idx].pwm			= pwmList[pwmListIndex];
			PwmListInfo[idx].stdGridCnt		= (uint8_t)LowStdGridCount;
			PwmListInfo[idx].OutHistLower	= (uint16_t)OutHistLower;
			PwmListInfo[idx].OutHistMiddle	= (uint16_t)OutHistMiddle;
			PwmListInfo[idx].OutHistUpper	= (uint16_t)OutHistUpper;

			pwmListIndex++;
			if (pwmListIndex <= (PWM_SEQUENCE_COUNT - 1))
			{
				//*PwmValue = pwmList[pwmListIndex];
				prev_pwmValue = (COMM_DUTY_CYCLE_LEVEL)pwmList[pwmListIndex];
				Livestatus = 3;//Only change pwm value
				CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)pwmList[pwmListIndex]);
				::Sleep(200);
				skipFrame1 = 2;
			}
			...
			{
				prev_pwmValue = (COMM_DUTY_CYCLE_LEVEL)DerivePwm[pwmListIndex];
				Livestatus = 11; //Spoof pwm value set
				CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)DerivePwm[pwmListIndex]);
				::Sleep(200);
				skipFrame1 = 2;
			}
		}
		...
		{
			Laplase_Score = computeLaplaseScore(in_inputImageData, in_Width, in_Height);
			//if (Laplase_Score <= SpoofParamThresold.Lapals_Score_Higher - 500)
			if (Quality <= 0)
			{
				Livestatus = 12; //Iris not found -capture stop
			}
			//else if (is_sortedTemperDevice(PwmListInfo, PWM_SEQUENCE_COUNT))
			//{
			//	if ((PwmListInfo[2].OutHistMiddle >= SpoofParamThresold.OutHistMiddle_2 && PwmListInfo[2].stdGridCnt < SpoofParamThresold.StdDevGreedCount) ||
			//		(PwmListInfo[2].OutHistLower > 11000 && PwmListInfo[2].OutHistMiddle < SpoofParamThresold.OutHistMiddle_1 && PwmListInfo[2].stdGridCnt < SpoofParamThresold.StdDevGreedCount) ||
			//		PwmListInfo[2].OutHistUpper >= SpoofParamThresold.OutHistUpper ||
			//		PwmListInfo[2].OutHistLower > 35000 || PwmListInfo[2].OutHistLower < 2500 ||
			//		PwmListInfo[2].OutHistMiddle < SpoofParamThresold.OutHistMiddle_3 || (Laplase_Score < SpoofParamThresold.Lapals_Score_Lower))
			//	{
			//		Livestatus = 10; //Place Finger Properly -capture continue
			//	}
			//	else
			//	{
			//		Livestatus = 4;
			...
			{
				if (is_sorted(PwmListInfo, PWM_SEQUENCE_COUNT))
				{
					Livestatus = 5;//Confirm Live-Normal  - capture stop
					ISComplete = true;
				}
				else
				{
					if (is_sortedHistSum(PwmListInfo, PWM_SEQUENCE_COUNT))
					{
						Livestatus = 6;//Confirm Live-LightSource  - capture stop
						ISComplete = true;
					}
					else
					{
						Livestatus = 7;//capture stop spoof detect

						//if (is_outdoorDevice(PwmListInfo, PWM_SEQUENCE_COUNT))
						//{//sun light
						//	Livestatus = 13;//Confirm Live-LightSource  - capture continue
						//}
						//else
						//{
						//	PwmRestCycleCntSpoof++;
						//	pwmListIndex = 0;
						//	Livestatus = 7; //Mfg Pwm Value Set and Start Capture flag = 1
						//}
					}
				}
			}

			if (PwmRestCycleCntSpoof >= 3)
			{
				pwmListIndex = 0;
				PwmRestCycleCntSpoof = 0;


				Livestatus = 8; //50% Temper - capture stop
			}
		}
	...
	FILE* fptr2 = NULL;
	fopen_s(&fptr2, "ImageSSPCheck.csv", "ab");
	fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.2f,%.2f,%d,%d\n", emp_id, curr_pwm, (uint8_t)Mean, (uint8_t)StdDev,
		Laplase_Score, (uint8_t)LowStdGridCount, (uint16_t)OutHistLower, (uint16_t)OutHistMiddle,
		(uint16_t)OutHistUpper, Livestatus,
		pEyeInfo.iris_center_x, pEyeInfo.iris_center_y, pEyeInfo.iris_diameter / 2, pEyeInfo.iris_quality,
		textureFeaturefullFrame,textureFeatureIrisFrame,texture_FF,texture_IRIS);
	//fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", emp_id, prev_pwmValue, (uint8_t)Mean, (uint8_t)StdDev, Laplase_Score, (uint8_t)LowStdGridCount, (uint16_t)OutHistLower, (uint16_t)OutHistMiddle, (uint16_t)OutHistUpper, Livestatus);
	fflush(fptr2);
	fclose(fptr2);
	return Livestatus;
...
DWORD WINAPI ImageProcessThreadFull(LPVOID lpParam)
{
	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			if (updown /*||  (g_ipl_info[0].pQtyInfo.Quality > 99)*/)
				ISComplete = true;
			...
			texture_IRIS = 0;
			/*GetTextureData_IRIS_FF(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT,
				g_ipl_info[0].pQtyInfo.X, g_ipl_info[0].pQtyInfo.Y, pEyeInfo.iris_diameter / 2, &texture_IRIS, &texture_FF);
			int retvalue = Getblur(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &mean, &blur);
			retvalue = GetMeanAndStddev(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &mean, &stddev);*/
			
			uint8_t SpoofStatus = 1;
			//if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0) && !updown && lowpwmhighmean < 6)
			if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0))
			{
				SpoofStatus = ImageSSPCheck(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, g_ipl_info[0].pQtyInfo.Quality,
				StartCapture_flag, &mean, &stddev, &blur,&IrisMean,true);
			}
			//else
			{
				//FILE* fptr2 = NULL;
				//fopen_s(&fptr2, "ImageSSPCheck.csv", "ab");
				//fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", emp_id, prev_pwmValue, (uint8_t)Mean, (uint8_t)StdDev,
				//	blur, Livestatus, pEyeInfo.iris_quality, texture_FF, texture_IRIS, IrisMean);
				...
				fprintf(fptr2, "%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.2f,%.2f,%d,%d\n", emp_id, prev_pwmValue, (uint8_t)Mean, (uint8_t)StdDev, 
					Laplase_Score, (uint8_t)LowStdGridCount, (uint16_t)OutHistLower, (uint16_t)OutHistMiddle,
					(uint16_t)OutHistUpper, Livestatus,
					pEyeInfo.iris_center_x, pEyeInfo.iris_center_y, pEyeInfo.iris_diameter/2,pEyeInfo.iris_quality,0.0,0.0,texture_FF,texture_IRIS);
				fflush(fptr2);
				fclose(fptr2);*/
			}
			...
			if(g_ipl_info[0].pQtyInfo.Quality > 0)
			{
				LogSpoofParameters(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &mean, &stddev, &blur);
			}
#endif
			...
			g_ipl_info[0].pQtyInfo.pupil_mean = texture_IRIS;//pEyeInfo.pupil_mean;
			g_ipl_info[0].pQtyInfo.pupil_blur = blur;
			g_ipl_info[0].pQtyInfo.iris_diameter = IrisMean;// white_count;
			g_ipl_info[0].pQtyInfo.pupil_diameter = mean;// black_count;
			g_ipl_info[0].pQtyInfo.sharpness = texture_FF;// pEyeInfo.sharpness;
			...
			unsigned char filePathname[2048] = { '\0' };
			sprintf((char*)&filePathname[0], "%d_FUlllFrame_%d.bmp", filecounter++, tempcount);
			WriteAllText1((const char*)filePathname, (char*)g_ipl_info[0].pBitmapImage.m_BitmapImage, g_ipl_info[0].nImageLengthWithBitmapHeader);
			...
			if (SpoofStatus == 12 || SpoofStatus == 7)
			{
				//g_ipl_info[0].pQtyInfo.Quality = 0;
				m_hCaptureCompleteThreadHandle = ::CreateThread(NULL, 0, &StopCapThread, 0, 0, NULL);
				IsGetQtyInCap = false;
				DeviceErrorCode = MARVIS_AUTH_E_FAKE_IRISFOUND;
				(m_CAPTURE_COMPLETE_CALLBACK)(DeviceErrorCode, &g_ipl_info[0].pQtyInfo, &g_ipl_info[0].pBitmapImage);
				IsAutoCapComplete = true;
				gCounter = 0;
				isDataBufferFilled = false;
...
int MARVIS_Auth_StartCapture(int in_quality, int TimeOut, PREVIEW_CALLBACK p_PREVIEW_CALLBACK, CAPTURE_COMPLETE_CALLBACK p_CAPTURE_COMPLETE_CALLBACK, char* empid)
{
	...
	try
	{
		...
		lowPwm = 0;
		updown = false;
		high_max_meanvalue = 0;
		high_min_meanvalue = 0;
		mid_meanvalue = 0;
		low_max_meanvalue = 0;
		low_min_meanvalue = 0;
		...
		memset(PwmListInfo, 0, sizeof(PwmListInfo));
		pwmListIndex = 0;
		...
		{
			// yellow
			Is_YellowOrpurple = false;
			prev_pwmValue = 16;
		}
		else
		{
			// purple
			Is_YellowOrpurple = true;
			prev_pwmValue = 14;
		}
		pwm_defaultValue = prev_pwmValue;
		PWM = 0;// prev_pwmValue;// COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17;
		//prev_pwmValue = COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17;
		retValue = CommSetIRLed((COMM_DUTY_CYCLE_LEVEL)prev_pwmValue);
		if (MARVIS_AUTH_E_SUCCESS != retValue)
		{
...
int MARVIS_Auth_PWMLevel(int pwmvalue)
{
	...
	int retValue = MARVIS_AUTH_E_SUCCESS;
	prev_pwmValue = pwmvalue;
	retValue = CommSetIRLed(COMM_DUTY_CYCLE_LEVEL(pwmvalue));
	if (MARVIS_AUTH_E_SUCCESS != retValue)
	{
		_snprintf_s(szOutputString, sizeof(szOutputString), sizeof(szOutputString),
			"%s - Line#[%d] ErrorCode#[%d] - CommStopCapture Fail.[%s]", __FUNCTION__, __LINE__,
