...
float m_gSimilarityScore[2][3];

int gSimilarityThreshold = 60;// 70;
...
int  Circledata(unsigned char *in_rawImg, int ImgWidth, int ImgHeight,
	int iris_center_x, int iris_center_y, int pupil_center_r, unsigned char* outdata, double * outBlur)
{
	...
	for (int i = StartY; i < height + StartY; i++)
	{
		for (int j = StartX; j < StartX + width; j++)
		{
			...
			if ((circleDist < (pEyeInfo.iris_diameter / 2)) && (i > pupil_y) && (i < pupil_y + pupil_height))
			{
	...
	double out_meanValue = sum / imageSize;
	...
	for (int i = StartY; i < height + StartY; i++)
	{
		for (int j = StartX; j < StartX + width; j++)
		{
			...
			if ((circleDist < (pEyeInfo.iris_diameter / 2)) && (i > pupil_y) && (i < pupil_y + pupil_height))
			{
				double diff = outdata[k] - out_meanValue;
	...
	*outBlur = sumSquaredDiff / imageSize;

	//WriteAllText1("111.raw", (char*)outdata, outlen * outlen);
	return white_pixel_count;
}

int Getblur(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, int* variance)
{
	double sum = 0;
...
DWORD WINAPI ImageProcessThreadFull(LPVOID lpParam)
{
	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			WriteAllText1("Final_Frame.raw", (char*)g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH*g_ipl_info[0].FINAL_FRAME_HEIGHT);
			...
			double  out_pupilBlur = 0;
			
			int white_count = Circledata(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT,
				g_ipl_info[0].pQtyInfo.X, g_ipl_info[0].pQtyInfo.Y, g_ipl_info[0].pQtyInfo.R, out_data, &out_pupilBlur);
			...
			//if (mean < 80)   //Condition 1
			//{
			//	g_ipl_info[0].pQtyInfo.Quality = 0;
			//}

			//
			//if ((mean >= 150 && mean  < 180) && (blur_value > 7000))
			//{
			//	g_ipl_info[0].pQtyInfo.Quality = 0;
			//}
			//else if ((mean >= 180 && mean  <= 200) && (blur_value > 6000))
			//{
			//	g_ipl_info[0].pQtyInfo.Quality = 0;
			//}
			//else if ((mean > 200 && mean  <= 210) && (blur_value > 5000))
			//{
			//	g_ipl_info[0].pQtyInfo.Quality = 0;
			//}
			//else if ((mean > 210 /*&& mean <= 210*/) && (blur_value > 4500))
			//{
			//	g_ipl_info[0].pQtyInfo.Quality = 0;
			//}

			g_ipl_info[0].pQtyInfo.pupil_blur = (int)blur_value;

			if (g_ipl_info[0].pQtyInfo.Quality > 0) 
			{
				
				int box_width = 20;
				int box_height = pEyeInfo.pupil_diameter;

				int Start_X = pEyeInfo.pupil_center_x - (pEyeInfo.iris_diameter / 2) - box_width - 5;
				int Start_Y = pEyeInfo.pupil_center_y - (box_height /2);
				float mean_box1 = 0;
				float mean_box2 = 0;
				if (((Start_X > 0) && (Start_X < g_ipl_info[0].FINAL_FRAME_WIDTH - box_width)) && ((Start_Y > 0) && (Start_Y < g_ipl_info[0].FINAL_FRAME_HEIGHT - box_height)))
				{
					mean_box1 = getMean_cropArea(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, Start_X, Start_Y, Start_X + box_width, Start_Y + box_height);

					Start_X = pEyeInfo.pupil_center_x + (pEyeInfo.iris_diameter / 2) + 5/* box_width*/;
					Start_Y = pEyeInfo.pupil_center_y - (box_height / 2);
					
					if (((Start_X > 0) && (Start_X < g_ipl_info[0].FINAL_FRAME_WIDTH - box_width)) && ((Start_Y > 0) && (Start_Y < g_ipl_info[0].FINAL_FRAME_HEIGHT - box_height)))
						mean_box2 = getMean_cropArea(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, Start_X, Start_Y, Start_X + box_width, Start_Y + box_height);
					else
						mean_box1 = 0;
				}
				...
				//g_ipl_info[0].pQtyInfo.pupil_blur = blur_value;//pEyeInfo.pupil_mean;
				g_ipl_info[0].pQtyInfo.pupil_diameter = abs(mean_box2 - mean_box1);
				g_ipl_info[0].pQtyInfo.percent_occlusion = mean;//mean_box2;
			...
			else
			{
				...
				//int retvalue = Getblur(out_data, pEyeInfo.iris_diameter, pEyeInfo.iris_diameter, &mean, &blur_value);
				int retvalue = Getblur(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &mean, &blur_value);
				//g_ipl_info[0].pQtyInfo.pupil_blur = 0;// blur_value;// mean_box1;// pEyeInfo.pupil_mean;
				g_ipl_info[0].pQtyInfo.percent_occlusion = mean;// mean_box2;
				g_ipl_info[0].pQtyInfo.pupil_diameter = 0;// abs(mean_box2 - mean_box1);
			...
			if (gCounter >= nMinCapRotation && !IsGetQtyInCap &&  g_ipl_info[0].pBitmapImage.m_BitmapImage != NULL && deviceStatus != DEVICE_STREAMING_STOPPED)
			{
				if (IsSimilarFrame)
				{
					...
					gCounter = 0;
					g_ipl_info[0].pQtyInfo.Quality = 0;
					gPreviewErrorCode = MARVIS_AUTH_E_FLIP_DEVICE;
					m_hCapturePreviewThreadHandle = ::CreateThread(NULL, 0, &CapturePreviewThread, 0, 0, NULL);
					}*/
				}
				else
...
int MARVIS_Auth_StartCapture(int in_quality, int TimeOut, PREVIEW_CALLBACK p_PREVIEW_CALLBACK, CAPTURE_COMPLETE_CALLBACK p_CAPTURE_COMPLETE_CALLBACK)
{
	char szOutputString[256] = { 0 };
