...
float max_mid_texture;
float min_mid_texture;
...
std::string getTodaysDateAsString()
{
	...
	// Convert to std::time_t to use with std::localtime
	...
	oss << std::put_time(&bt, "%d%m%Y%H%M%S%MS") << '.' << std::setfill('0') << std::setw(3) << ms.count();
	std::string timestamp = oss.str();

	return timestamp;
}

void writeToFile_com1(const char *filename, const void *data, size_t size) {
	FILE *file = fopen(filename, "ab"); // Open the file in append binary mode
	if (file != NULL) {
		fwrite(data, 1, size, file); // Write the data to the file
	...
	int iris_center_x, int iris_center_y, int pupil_center_r, unsigned char* outdata, double *out_black_count)
{
	...
	int pupil_y = iris_center_y - pupil_center_r;
	int pupil_height = pupil_center_r * 2;
	int pixelvaluecount = 0,pixelvalue;
	int  k = 0;
	int outlen = pupil_center_r * 2;
	memset(outdata, 0, outlen*outlen);
	double sum = 0;
	...
	for (int i = StartY; i < height + StartY; i++)
	{
		for (int j = StartX; j < StartX + width; j++)
		{
			...
			if ((circleDist < (pupil_center_r)) && (i > pupil_y) && (i < pupil_y + pupil_height))
			{
				...
				//in_rawImg[l] = 0;
				sum += in_rawImg[l];
				pixelvalue = in_rawImg[l];
				if (pixelvalue > 235)
					pixelvaluecount++;
				//if (pixelvaluecount > 1500)
...
void NormalizeImage(unsigned char* imageData, int width, int height) 
{
	double sum = 0;
	...
	double mean = sum / imageSize;

	double sumSquaredDiff = 0;
	for (int i = 0; i < imageSize; ++i) 
	{
		double diff = imageData[i] - mean;
		sumSquaredDiff += diff * diff;
	}
	double stddev = sqrt(sumSquaredDiff / imageSize);

	for (int i = 0; i < imageSize; ++i) 
	{
		//imageData[i] = (unsigned char)((imageData[i] - mean) / stddev * 128 + 128);
		double normalized = (imageData[i] - mean) / stddev * 64 + 64; // Adjusted scaling factor
		imageData[i] = static_cast<unsigned char>(my_min(my_max(normalized, 0.0), 255.0));
		//imageData[i] = (unsigned char)std::min(std::max(normalized, 0.0), 255.0);
	}
}
...
bool GetSpoofStatus(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	//NormalizeImage(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
	...
	stcimgeproperty.iris_mean = mean;
	stcimgeproperty.iris_sd = std_dev;
	stcimgeproperty.texture = textureFeature;
	stcimgeproperty.quality = g_ipl_info[0].pQtyInfo.Quality;
	stcimgeproperty.oclude = pEyeInfo.percent_occlusion;
	stcimgeproperty.sharp = pEyeInfo.sharpness;
	stcimgeproperty.similarity = m_gSimilarityScore[0];
	...
	//	if (max_mid_texture < irPropety[irPropety.size() - 1].texture)
	//	{
	//		/*if (min_high_texture == 0)
	//		{
	//		min_high_texture = max_high_texture;
	//		}*/
	//		max_mid_texture = irPropety[irPropety.size() - 1].texture;
	//	}

	//	if ((min_mid_texture > irPropety[irPropety.size() - 1].texture) || min_mid_texture == 0)
	//		min_mid_texture = irPropety[irPropety.size() - 1].texture;
	//}
	...
	if ((skipFrame > 1) && (highPwm >= 3) && (!IsSpoofStatus))
	{
		if (max_high_texture - min_high_texture > 10)
		{
			...
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%f,%d,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "timeout", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].iris_mean, irPropety[i].iris_sd, irPropety[i].quality, irPropety[i].similarity, irPropety[i].oclude, irPropety[i].sharp);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);
		}
	...
	/*if ((skipFrame > 2) && (midPwm >3) && (!IsSpoofStatus))
	{
		if (max_mid_texture - min_mid_texture > 10)
		{
			skipFrame = 0;
		}
		else
	...
	if ((skipFrame > 1) && (lowPwm >= 3))
	{
		...
		if (max_low_texture - min_low_texture > 10)
		{
			...
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%f,%d,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "timeout", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].iris_mean, irPropety[i].iris_sd, irPropety[i].quality, irPropety[i].similarity, irPropety[i].oclude,  irPropety[i].sharp);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);
		}
		else if ((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>30 && ((high_meanvalue > low_meanvalue))/* && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 40)*/)
		{
			...
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%f,%d,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].iris_mean, irPropety[i].iris_sd, irPropety[i].quality, irPropety[i].similarity, irPropety[i].oclude, irPropety[i].sharp);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);
		}
	...
	if (lowpwmhighmean >= 6)
	{
		...
		for (int i = 0; i < irPropety.size(); i++)
		{
			fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%f,%d,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "Not Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].iris_mean, irPropety[i].iris_sd, irPropety[i].quality, irPropety[i].similarity, irPropety[i].oclude, irPropety[i].sharp);
		}
		fprintf(fptr1, "\n");
		fclose(fptr1);
...
*/
			...
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%f\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "normal", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].iris_mean, irPropety[i].iris_sd, irPropety[i].quality, irPropety[i].similarity);
			}
			fprintf(fptr1,"");
			fclose(fptr1);
			*/
			...
			////////////////////////////////////////
			/*char strExt[500];
			string curTime = getTodaysDateAsString();
			string cTimeStr(curTime.c_str());

			_snprintf_s(strExt, sizeof(strExt), "%s.bmp", curTime.c_str());*/
			...
			//if (fp == NULL) 
			//{
			//	std::cerr << "Error opening file: " << strExt << std::endl;
			//	return 1; // Exit with error
			//}
			fwrite(g_ipl_info[0].Final_Frame, (g_ipl_info[0].FINAL_FRAME_HEIGHT*g_ipl_info[0].FINAL_FRAME_WIDTH), 1,  fp);
			fclose(fp);	

			//writeToFile_com1("frames.raw", g_ipl_info[0].Final_Frame, (g_ipl_info[0].FINAL_FRAME_HEIGHT*g_ipl_info[0].FINAL_FRAME_WIDTH));
			////////////////////////////////////////////////
