...
bool GetSpoofStatus(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL11))
	{
		...
		if (max_low_texture - min_low_texture > 10 || (max_low_meanvalue - min_low_meanvalue>20))
		{
			...
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "timeout", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].sharpness);
				//fprintf(fptr1, "%s,%d,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%lf\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].sd, irPropety[i].blur, irPropety[i].texture, "timeout", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_circle);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);
		...
		else if (((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>30) && (max_high_meanvalue > max_low_meanvalue)/* && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 20)*/ && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 2))
	...
	{
		if ((max_high_meanvalue > max_low_meanvalue) && ((average_high_mean/*irPropety[0].mean */ - average_low_mean /*irPropety[irPropety.size() - 1].mean*/) >= 10) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 2))
...
bool GetSpoofStatus1(unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur)
{
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL14))
	{
		...
		else if (((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>5) && (max_high_meanvalue > max_low_meanvalue)/* && ((irPropety[0].mean - irPropety[irPropety.size() - 1].mean) >= 20)*/ && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 2))
		{
	...
	if (lowpwmhighmean >= 1 && lowpwmhighmean < 6)
	{
		if ((max_high_meanvalue > max_low_meanvalue) && ((average_high_mean/*irPropety[0].mean*/ - average_low_mean /*irPropety[irPropety.size() - 1].mean*/) >= 20) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 2))
		{
