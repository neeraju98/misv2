...
int  average_textureff = 0, average_textureiris = 0, average_high_mean = 0, average_low_mean = 0, average_magnitude = 0;
int mean_count = 0, mean_count_1 = 0;
bool no_quality = false;
string lens_type = "";
char* gModelBuffer = NULL;
int gModelBufferLen = 0;
int g_Capture_Type=-1;
char* gLabelBuffer = NULL;
int gLabelBufferLen = 0;
//bool status = false;

#pragma region Variable Declaration

int maximum = 0; double textureFeature;
int minimum = 0;
int First = 0;
bool IRLed_Status = true;
bool IRLed_On = false;
bool IRLed_First = false;
bool SpoofStatus = false;
...
int DeviceErrorCodeCaptureComplete = 0;
...
double endtime = 0;
bool updown = 0;
int max_high_meanvalue;
int max_low_meanvalue;
int min_high_meanvalue;
int min_low_meanvalue;
int max_texture_iris_count;
float max_lap, min_lap;
...
struct imgProperty {
	...
	int blur;
	double texture;
	double texture_circle;
	int texture_iriscount;
	float pupil_mean, iris_mean;
	int texture_framecount;
	int sharpness;
	float  laplacian_count;
	float avg_magnitude;
...
int MSSD_LoadFile(char* modelPath, char *out_modelBuffer, int &out_modelBufferLen)
{
	if (modelPath == NULL)
	{
		return -1;
	...
	//in_modelBuffer = (char*)calloc(size, sizeof(char));
	out_modelBufferLen = size;

	size_t bytes_read = fread(out_modelBuffer, sizeof(unsigned char), out_modelBufferLen, file);
	fclose(file);
...
void GetTFLite_Model_Label(char **out_modelBuffer, int &out_modelBufferLen, char **out_labelBuffer, int &out_labelBufferLen)
{

	*out_modelBuffer = new char[23363080 + 1024];
	*out_labelBuffer = new char[100];


	(char*)MSSD_LoadFile((char*)"model\\tflite_quantized\\model.tflite", *out_modelBuffer, out_modelBufferLen);
	(char*)MSSD_LoadFile((char*)"model\\tflite_quantized\\labels.txt", *out_labelBuffer, out_labelBufferLen);

	/**out_modelBuffer = (char*)MSSD_LoadFile((char*)"model\\tflite_quantized\\model.tflite", out_modelBufferLen);
	*out_labelBuffer = (char*)MSSD_LoadFile((char*)"model\\tflite_quantized\\labels.txt", out_labelBufferLen);*/
}
...
void readFromFile(const char *name, unsigned char *data, int *datalength)
{
	try
	{
		int ReadFile = 0;
...
double *apply_sobel(unsigned char *img, int width, int height)
{
	int x, y;
	int gx = 0, gy = 0;

	double *magnitude;
	magnitude = (double *)malloc(width * height * sizeof(double));
	try
	{
		...
		for (y = 1; y < height - 1; y++)
		{
			for (x = 1; x < width - 1; x++)
			{
				...
				int mid_left = y * width + (x - 1);
				int mid_right = y * width + (x + 1);

				int bot_left = (y + 1) * width + (x - 1);
				int bot = (y + 1) * width + x;
				int bot_right = (y + 1) * width + (x + 1);

				// Calculate gx

				gx = -(img[top_left]) + (img[top_right])
					- 2 * (img[mid_left]) + 2 * (img[mid_right]) - (img[bot_left]) + (img[bot_right]);

				// Calculate gy
				gy = (img[top_left]) + 2 * (img[top]) + (img[top_right])
					- (img[bot_left]) - 2 * (img[bot]) - (img[bot_right]);

				// Store magnitude
				magnitude[y * width + x] = sqrt((double)(gx * gx + gy * gy));


				//magnitude = sqrt((double)(gx * gx + gy * gy));
			}
		}
		return magnitude;
	}
...
int GetTextureData_IRIS_FF(unsigned char* RawData, int width, int height, int Centre_X, int Centre_Y, int Circle_Radius,
...
float  Laplacian_count(unsigned char* RawData, int width, int height)
{
	double laplacian = 0.0, output = 0.0;
	for (size_t i = 1; i < height; ++i)
	{
		for (size_t j = 1; j < width; ++j)
		{
			...
			int val = abs(8 * RawData[i*width + j] - RawData[(i - 1)*width + j] - RawData[(i + 1)*width + j] - RawData[i*width + (j - 1)] - RawData[i*width + (j + 1)] - RawData[(i - 1)*width + (j - 1)] - RawData[(i - 1)*width + (j + 1)] - RawData[(i + 1)*width + (j - 1)] - RawData[(i + 1)*width + (j + 1)]);
			laplacian += val;

		}
	}
	output = laplacian / (height*width);
	return output;
}
...
{
	//unsigned char *output_iris = (unsigned char*)malloc(g_ipl_info[0].FINAL_FRAME_WIDTH* g_ipl_info[0].FINAL_FRAME_HEIGHT);
...
bool GetSpoofStatus_PurpleLens()
{
	if (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL16)
	{
		skipFrame++;
	...
	int sum_textureff = 0, sum_textureiris = 0, sum_avg_mag = 0;
	float lap_c = 0.0;
	imgProperty stcimgeproperty;
	int sum = 0;
	float avg_mag = 0;
	double *magnitude = NULL;
	magnitude = apply_sobel(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);

	int count = (g_ipl_info[0].FINAL_FRAME_WIDTH - 2) * (g_ipl_info[0].FINAL_FRAME_HEIGHT - 2); // Number of valid pixels
	...
	int ret = GetTextureData_IRIS_FF(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, pEyeInfo.iris_center_x, pEyeInfo.iris_center_y, pEyeInfo.iris_diameter / 2, &Innercount, &OuterCount);
	...
	stcimgeproperty.avg_magnitude = avg_mag;
	stcimgeproperty.texture_iriscount = Innercount;
	stcimgeproperty.texture_framecount = OuterCount;
	stcimgeproperty.iris_mean = pEyeInfo.iris_mean;
	stcimgeproperty.pupil_mean = pEyeInfo.pupil_mean;
	stcimgeproperty.sharpness = g_ipl_info[0].pQtyInfo.sharpness;


	irPropety.push_back(stcimgeproperty);


	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17))
	{
		skipFrame++;
		if (max_high_meanvalue < irPropety[irPropety.size() - 1].mean)
			max_high_meanvalue = irPropety[irPropety.size() - 1].mean;

		if ((min_high_meanvalue  > irPropety[irPropety.size() - 1].mean) || min_high_meanvalue == 0)
			min_high_meanvalue = irPropety[irPropety.size() - 1].mean;
		...
		{
			sum_high_mean += irPropety[i].mean;
		}
		average_high_mean = sum_high_mean / irPropety.size();

	}

	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL11))
	{
		skipFrame++;
		if (max_low_meanvalue < irPropety[irPropety.size() - 1].mean)
			max_low_meanvalue = irPropety[irPropety.size() - 1].mean;

		if ((min_low_meanvalue > irPropety[irPropety.size() - 1].mean) || min_low_meanvalue == 0)
			min_low_meanvalue = irPropety[irPropety.size() - 1].mean;
		...
		for (int i = 0; i < irPropety.size(); i++)
		{
			if (irPropety[i].irValue == 11)
			{
				++mean_count_1;
				sum_low_mean += irPropety[i].mean;
			}
		}

		average_low_mean = sum_low_mean / mean_count_1;
	}
	...
	for (int i = 0; i < irPropety.size(); i++)
	{
		...
		sum_textureff += irPropety[i].texture_framecount;
		sum_textureiris += irPropety[i].texture_iriscount;
		sum_avg_mag += irPropety[i].avg_magnitude;
	}
	average_textureff = sum_textureff / irPropety.size();
	average_textureiris = sum_textureiris / irPropety.size();
	average_magnitude = sum_avg_mag / irPropety.size();

	if (max_texture_iris_count < irPropety[irPropety.size() - 1].texture_iriscount)
		max_texture_iris_count = irPropety[irPropety.size() - 1].texture_iriscount;





	if (irPropety[irPropety.size() - 1].pupil_mean == 0)
	{
		irPropety[irPropety.size() - 1].pupil_mean = 1.0;
	}

	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17))
	{
		if (max_high_texture - min_high_texture > 10 || (max_high_meanvalue - min_high_meanvalue > 20))
		{
			...
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].blur, irPropety[i].texture, "spoof case", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].laplacian_count, max_lap, min_lap, irPropety[i].avg_magnitude, average_magnitude);
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL11))
	{
		if (max_low_texture - min_low_texture > 10 || (max_low_meanvalue - min_low_meanvalue > 20))
		{
			skipFrame = 0;
			FILE* fptr1 = NULL;
			fopen_s(&fptr1, "Image_Properties.csv", "ab");
			for (int i = 0; i < irPropety.size(); i++)
			{
				fprintf(fptr1, "%s,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].blur, irPropety[i].texture, "spoof case", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].laplacian_count, max_lap, min_lap, irPropety[i].avg_magnitude, average_magnitude);
			}
			fprintf(fptr1, "\n");
			fclose(fptr1);
			lowpwmhighmean = 6;
		...
		else if (((irPropety[0].texture - irPropety[irPropety.size() - 1].texture) > 30) && (max_high_meanvalue > max_low_meanvalue) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && average_magnitude > 25/*(max_lap - min_lap)< 15*/)
		{
			if ((average_textureff > 200))
			{
				lowpwmhighmean = 0;

				retvalue = CommSetIRLed(COMM_DUTY_CYCLE_LEVEL16);
				::Sleep(200);
				prev_pwmValue = COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL16;
				skipFrame = 0;
				//status = true;
				...
				for (int i = 0; i < irPropety.size(); i++)
				{
					fprintf(fptr1, "%s,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].blur, irPropety[i].texture, "Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].laplacian_count, max_lap, min_lap, irPropety[i].avg_magnitude, average_magnitude);
				}
				fprintf(fptr1, "\n");
				fclose(fptr1);
			}
	...
	if (lowpwmhighmean >= 1 && lowpwmhighmean < 6)
	{
		if ((max_high_meanvalue > max_low_meanvalue) && ((average_high_mean - average_low_mean) >= 10) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && average_magnitude > 25 /*(max_lap - min_lap)< 15*/)
		{
			if ((average_textureff > 200))
			{
				lowpwmhighmean = 0;
	...
	if (lowpwmhighmean >= 6)
	{
		...
		for (int i = 0; i < irPropety.size(); i++)
		{
			fprintf(fptr1, "%s,%d,%d,%d,%lf,%s,%lf,%lf,%lf,%lf,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d\n", emp_id, irPropety[i].irValue, irPropety[i].mean, irPropety[i].blur, irPropety[i].texture, "Not Captured", max_high_texture, min_high_texture, max_low_texture, min_low_texture, irPropety[i].texture_framecount, irPropety[i].texture_iriscount, average_textureff, max_texture_iris_count, max_high_meanvalue, min_high_meanvalue, max_low_meanvalue, min_low_meanvalue, average_high_mean, average_low_mean, irPropety[i].iris_mean, irPropety[i].pupil_mean, irPropety[i].iris_mean / irPropety[i].pupil_mean, irPropety[i].laplacian_count, max_lap, min_lap, irPropety[i].avg_magnitude, average_magnitude);
		}
		fprintf(fptr1, "\n");
		fclose(fptr1);
...
bool GetSpoofStatus_YellowLens()
{
	if (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL16)
	{
		skipFrame++;
	...
	int Innercount = 0, OuterCount = 0;
	float lap_c = 0.0;
	int sum_textureff = 0, sum_textureiris = 0;
	imgProperty stcimgeproperty;
	float avg_mag = 0;
	int sum = 0;
	...
	int ret = GetTextureData_IRIS_FF(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, pEyeInfo.iris_center_x, pEyeInfo.iris_center_y, pEyeInfo.iris_diameter / 2, &Innercount, &OuterCount);
	textureFeature = CalculateTextureFeature(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
	//lap_c = Laplacian_count(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
	...
	stcimgeproperty.pupil_mean = pEyeInfo.pupil_mean;
	stcimgeproperty.sharpness = g_ipl_info[0].pQtyInfo.sharpness;
	//stcimgeproperty.laplacian_count = lap_c;
	...
	if ((prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL14))
	{
		skipFrame++;
		if (max_low_meanvalue < irPropety[irPropety.size() - 1].mean)
			max_low_meanvalue = irPropety[irPropety.size() - 1].mean;
	...
	/*if (max_lap < irPropety[irPropety.size() - 1].laplacian_count)
		max_lap = irPropety[irPropety.size() - 1].laplacian_count;

		if ((min_lap  > irPropety[irPropety.size() - 1].laplacian_count) || min_lap == 0)
		min_lap = irPropety[irPropety.size() - 1].laplacian_count;*/
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL17))
	{
		...
		{
			retvalue = CommSetIRLed(COMM_DUTY_CYCLE_LEVEL14);
			prev_pwmValue = COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL14;
			::Sleep(200);
			skipFrame = 0;
			no_quality = true;
		}
	...
	if ((skipFrame > 1) && (prev_pwmValue == COMM_DUTY_CYCLE_LEVEL::COMM_DUTY_CYCLE_LEVEL14))
	{
		...
		else if (((irPropety[0].texture - irPropety[irPropety.size() - 1].texture)>5) && (max_high_meanvalue > max_low_meanvalue) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && average_magnitude > 25 /*(max_lap-min_lap)< 10*/)
		{
			if ((average_textureff > 200))
			{
				lowpwmhighmean = 0;
	...
	if (lowpwmhighmean >= 1 && lowpwmhighmean < 6)
	{
		if ((max_high_meanvalue > max_low_meanvalue) && ((average_high_mean - average_low_mean) >= 20) && (average_textureff <= 10000 && max_texture_iris_count <= 2000) && (irPropety[irPropety.size() - 1].blur > 500) && (irPropety[irPropety.size() - 1].blur < 7000) && ((irPropety[irPropety.size() - 1].iris_mean / irPropety[irPropety.size() - 1].pupil_mean) > 1.5) && average_magnitude > 25/* (max_lap - min_lap)< 10*/)
		{
			if ((average_textureff > 200))
			{
				lowpwmhighmean = 0;
...
//Live: true, Fake: false
bool GetModelStatus()
{
	int temp_out_IsLive[2] = { 0 };
	float temp_out_score[2] = { 0 };
	int retValue = MARVIS_AUTH_E_SUCCESS;
	bool ModelStatus = false;
	try
	{
		retValue = MSSD_CheckLiveness_imgPreprocess(gModelBuffer, gModelBufferLen, true,
			(char*)g_ipl_info[0].pCompleteFrame.Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT,
			temp_out_IsLive, temp_out_score, 2, modeltype::Teachable_Machine);
		if (retValue == 0)
		{
			if (temp_out_IsLive[0] == 0)
			{
				ModelStatus = false;

				g_ipl_info[0].pCompleteFrame.pQtyInfo.percent_occlusion = temp_out_IsLive[0];
				g_ipl_info[0].pCompleteFrame.pQtyInfo.iris_mean = temp_out_IsLive[1];
				g_ipl_info[0].pCompleteFrame.pQtyInfo.sclera_mean = 0;
				g_ipl_info[0].pCompleteFrame.pQtyInfo.sharpness = 0;
			}
			else
			{
				if (temp_out_score[0]>MODEL_THRESH)
					ModelStatus = true;
				else
				{
					ModelStatus = false;
				...
				g_ipl_info[0].pCompleteFrame.pQtyInfo.percent_occlusion = temp_out_IsLive[0];
				g_ipl_info[0].pCompleteFrame.pQtyInfo.iris_mean = temp_out_IsLive[1];
				g_ipl_info[0].pCompleteFrame.pQtyInfo.sclera_mean = temp_out_score[0];
				g_ipl_info[0].pCompleteFrame.pQtyInfo.sharpness = temp_out_score[1];

			}
		}
		FILE* fptr1 = NULL;
		fopen_s(&fptr1, "Model.csv", "ab");
		fprintf(fptr1, "%d,%d,%f,%f,%d\n", g_ipl_info[0].pQtyInfo.Quality, temp_out_IsLive[0], temp_out_score[0], temp_out_score[1], updown);
		fprintf(fptr1, "\n");
		fclose(fptr1);
...
DWORD WINAPI CompleteCapThread(LPVOID lpParam)
{
	...
	else
	{
		if (m_CAPTURE_COMPLETE_CALLBACK)
		{
			(m_CAPTURE_COMPLETE_CALLBACK)(DeviceErrorCodeCaptureComplete, &g_ipl_info[0].pCompleteFrame.pQtyInfo, &g_ipl_info[0].pCompleteFrame.pBitmap);
		}
	}
	return 0;
}
...
DWORD WINAPI ImageProcessThreadFull(LPVOID lpParam)
{
	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			iplReadImage(g_ipl_info[0].RawImageBuffer, "MN00_18092024132342.bmp", g_ipl_info[0].FULL_FRAME_WIDTH, g_ipl_info[0].FULL_FRAME_HEIGHT, 0);
...
#ifdef FPS_FINAL_MODE

			iplReadImage(g_ipl_info[0].Final_Frame, "LeftIris24092024150420.bmp", g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, 0);
			unsigned char *rt = rotate180(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT);
#endif
			...
			if (g_Capture_Type!=0 && g_Capture_Type!=3)
			{
				//Spoof logic start
				if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0 || no_quality) && !SpoofStatus)
				{
					if (lowpwmhighmean < 6)
					{
						if (lens_type=="YELLOW")
							SpoofStatus = GetSpoofStatus_YellowLens();
						else
							SpoofStatus = GetSpoofStatus_PurpleLens();
					}
					else
					{
						SpoofStatus = false;
			...
			g_ipl_info[0].pQtyInfo.iris_diameter = pEyeInfo.iris_diameter;
			g_ipl_info[0].pQtyInfo.pupil_diameter = pEyeInfo.pupil_diameter;
			...
			else if (lowpwmhighmean >= 6 && !SpoofStatus)
			{
				std::memcpy(g_ipl_info[0].pCompleteFrame.Final_Frame, g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH * g_ipl_info[0].FINAL_FRAME_HEIGHT);
			...
			{
				if (SpoofStatus || DeviceErrorCode == MARVIS_AUTH_E_FAKE_IRISFOUND)
				{
					IsGetQtyInCap = true;
					...
					{
						//model
						if (DeviceErrorCode != MARVIS_AUTH_E_FAKE_IRISFOUND )
						{
							if (g_Capture_Type==1 || g_Capture_Type==3)
							{
								if (GetModelStatus())
								{
									DeviceErrorCode = MARVIS_AUTH_E_SUCCESS;
								...
								{
									DeviceErrorCode = MARVIS_AUTH_MODEL_FAKE;
								}
							}
						}
						DeviceErrorCodeCaptureComplete = DeviceErrorCode;
...
int MARVIS_Auth_UninitDevice()
{
	...
	freeBuffer();
	freeIPLLibVariables();
	if (gModelBuffer != NULL)
	{
		free(gModelBuffer);
		gModelBuffer = NULL;
	}

	if (gLabelBuffer != NULL)
	{
		free(gLabelBuffer);
		gLabelBuffer = NULL;
	}
...
int MARVIS_Auth_InitDevice(int fd, const char *pcProductName, int ProductNameLen, MARVIS_DEVICE_INFO *StDeviceInfo)
{
	...
	try
	{
		...
		if (IsDeviceInit == true)
		{
			_snprintf_s(szOutputString, sizeof(szOutputString), sizeof(szOutputString),
				"%s - Line#[%d] ErrorCode#[%d] -[%s]", __FUNCTION__, __LINE__,
				MARVIS_AUTH_E_ALREADY_INIT, MARVIS_Auth_GetErrDescription(MARVIS_AUTH_E_ALREADY_INIT));
			LOG_ERROR(szOutputString);
			return MARVIS_AUTH_E_ALREADY_INIT;
		}
		...
		unsigned char *out_pFirmwareVersionDetails = (unsigned char*)malloc(COMM_FIRMWARE_VERSION_INFO_LENGTH);
		unsigned char *out_pFirmwareVersion = (unsigned char*)malloc(12);
		int out_pFirmwareVersionDetailsLen = 0;
		
		int ret = CommReadFirmwareVersion(out_pFirmwareVersionDetails, &out_pFirmwareVersionDetailsLen);
		memcpy(out_pFirmwareVersion, out_pFirmwareVersionDetails, 11);
		out_pFirmwareVersion[11] = '\0';
		if ((strcmp((char*)out_pFirmwareVersion, "03.03.03.01") == 0) || (strcmp((char*)out_pFirmwareVersion, "03.03.03.02") == 0))
			lens_type = "YELLOW"; 
		else
			lens_type = "BLUE";


		unsigned char* in_FirmwareData = NULL;
		int in_FirmwaredataLen = 5;
		in_FirmwareData = (unsigned char*)malloc(10);
		//memset(in_FirmwareData, '1', 10);
		if (!(strcmp((char*)out_pFirmwareVersion, "03.03.03.02") == 0 || strcmp((char*)out_pFirmwareVersion, "03.03.01.17") == 0))
		{
			retValue = CommLoadFirmware(in_FirmwareData, in_FirmwaredataLen, out_pFirmwareVersionDetailsLen);
			if (MARVIS_AUTH_E_SUCCESS != retValue)
			{
				_snprintf_s(szOutputString, sizeof(szOutputString), sizeof(szOutputString),
					"%s - Line#[%d] ErrorCode#[%d] - CommLoadFirmware Fail.[%s]", __FUNCTION__, __LINE__,
					retValue, MARVIS_Auth_GetErrDescription(retValue));
				LOG_ERROR(szOutputString);
				return retValue;
			}
		}
		if (out_pFirmwareVersionDetails != NULL)
		{
			free(out_pFirmwareVersionDetails);
			out_pFirmwareVersionDetails = NULL;
		}
		if (out_pFirmwareVersion != NULL)
		{
			free(out_pFirmwareVersion);
			out_pFirmwareVersion = NULL;
		}
		...
		int modelBufferLen = 0;
		char* labelBuffer = NULL;
		int labelBufferLen = 0;



		GetTFLite_Model_Label(&modelBuffer, modelBufferLen, &labelBuffer, labelBufferLen);
		retValue = MSSD_CheckLoadModel(modelBuffer, modelBufferLen);
		if (MARVIS_AUTH_E_SUCCESS != retValue)
		{
			_snprintf_s(szOutputString, sizeof(szOutputString), sizeof(szOutputString),
				"%s - Line#[%d] ErrorCode#[%d] - Model Load Fail.[%s]", __FUNCTION__, __LINE__,
				retValue, MARVIS_Auth_GetErrDescription(retValue));
			LOG_ERROR(szOutputString);
			return retValue;
		}

		gModelBuffer = (char*)calloc(modelBufferLen + 1, sizeof(char));
		memcpy(gModelBuffer, modelBuffer, modelBufferLen);
		gModelBufferLen = modelBufferLen;

		gLabelBuffer = (char*)calloc(labelBufferLen + 1, sizeof(char));
		memcpy(gLabelBuffer, labelBuffer, labelBufferLen);
		gLabelBufferLen = labelBufferLen; 
...
int MARVIS_Auth_StartCapture(int in_quality, int TimeOut, PREVIEW_CALLBACK p_PREVIEW_CALLBACK, CAPTURE_COMPLETE_CALLBACK p_CAPTURE_COMPLETE_CALLBACK, char* empid,int Capture_type)
{
	char szOutputString[256] = { 0 };
	...
	try
	{
		...
		prev_pwmValue = 0;
		updown = false;
		max_high_meanvalue = 0;
		min_low_meanvalue = 0;
		max_texture_iris_count = 0;
		max_lap = 0, min_lap = 0;
		min_high_meanvalue = 0;
		average_high_mean = 0, average_low_mean = 0, average_magnitude = 0;
		max_low_meanvalue = 0;
		no_quality = false;
...
const char *MARVIS_Auth_GetErrDescription(int errorNo)
{
	...
	else
	{
		...
		case MARVIS_AUTH_MODEL_FAKE:
			return "Model Not Approved";
		case MARVIS_AUTH_E_FAKE_IRISFOUND:
			return /*"Iris not Approved"*/"Iris capture was unstable. Please try again.";
		case MARVIS_AUTH_E_ALREADY_INIT:
			return "Device Already Initialized";
		case MSSD_FAILED_TO_LOAD_MODEL:
			return "Fail To Load To Model";
		default:
