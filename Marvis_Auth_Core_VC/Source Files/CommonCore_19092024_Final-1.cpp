...
bool GetSpoofStatus(/*unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur*/)
{
	...
	//int retvalue = GetMeanAndStddev(g_ipl_info[0].Final_Frame, g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, &meanValue, &stdValue);
...
bool GetSpoofStatus1(/*unsigned char* in_inputImageData, int in_Width, int in_Height, double* out_meanValue, double* out_stddev, double *out_Blur*/)
{
	bool IsSpoofStatus = false;
	int  sum_high_mean = 0;
	int  sum_low_mean = 0;
...
DWORD WINAPI ImageProcessThreadFull(LPVOID lpParam)
{
	...
	try
	{
		...
		if (IsDeviceInit && isDataBufferFilled && deviceStatus != DEVICE_STREAMING_STOPPED)
		{
			...
			iplReadImage(g_ipl_info[0].Final_Frame, "MN00_UnStable_18092024150749.bmp", g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, 0);
			...
			/*int len = 0;
			readFromFile("C:\\Users\\Neeraj.Upadhyay\\Desktop\\mssd\\MN00_05092024173432.raw", g_ipl_info[0].Final_Frame, &len);*/
			...
			if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0 || no_quality) && !status && lowpwmhighmean < 6)
			{
				if (nMinQty == 55)//yellow 
					SpoofStatus = GetSpoofStatus1();


				else
					SpoofStatus = GetSpoofStatus();
			}
			else
			{
				skipFrame = 0;
			...
			//if (lowpwmhighmean >= 6 || validframecounter > 5)
			if (lowpwmhighmean >= 6)
			{
				...
				if (!IsAutoCap)
				{
					...
					retValue = iplConvertRawToBmp(g_ipl_info[0].Final_Frame,
						g_ipl_info[0].pBitmapImage.m_BitmapImage,
						g_ipl_info[0].FINAL_FRAME_HEIGHT, g_ipl_info[0].FINAL_FRAME_WIDTH, 0);
					...
					(m_CAPTURE_COMPLETE_CALLBACK)(DeviceErrorCode, &g_ipl_info[0].pCompleteFrame.pQtyInfo, &g_ipl_info[0].pBitmapImage);
				}
			...
			//g_ipl_info[0].pQtyInfo.iris_mean = /*pEyeInfo.iris_mean*/temp_out_IsLive[1];
			//g_ipl_info[0].pQtyInfo.sclera_mean = /*pEyeInfo.pupil_mean*/temp_out_score[0];
			//g_ipl_info[0].pQtyInfo.sharpness = /*blur*/temp_out_score[1];
			...
			/////////////////////////////////////////////
			//	FILE* fp = NULL;
			//	string name = "frames" + to_string(prev_pwmValue) + ".raw";
						...
						if (IsSimilarity && (g_ipl_info[0].pQtyInfo.Quality > 0))
						{
							try
							{
								retValue = MSSD_CheckLiveness_imgPreprocess(gModelBuffer, gModelBufferLen, true, 
									(char*)g_ipl_info[0].Final_Frame,
									g_ipl_info[0].FINAL_FRAME_WIDTH, g_ipl_info[0].FINAL_FRAME_HEIGHT, temp_out_IsLive, temp_out_score, 2, modeltype::Teachable_Machine);
								...
								g_ipl_info[0].pCompleteFrame.pQtyInfo.percent_occlusion = /*pEyeInfo.percent_occlusion;*//*m_NoOfFrameCount*/temp_out_IsLive[0];
								g_ipl_info[0].pCompleteFrame.pQtyInfo.iris_mean = /*pEyeInfo.iris_mean*/temp_out_IsLive[1];
								g_ipl_info[0].pCompleteFrame.pQtyInfo.sclera_mean = /*pEyeInfo.pupil_mean*/temp_out_score[0];
								g_ipl_info[0].pCompleteFrame.pQtyInfo.sharpness = /*blur*/temp_out_score[1];
								/*if (temp_out_IsLive[0] == 1 /*&& temp_out_score[1] > 30)
								//{
								//	updown = true;
								//}
								//else*/

								if (temp_out_IsLive[0] == 0 /*&& temp_out_score[0]< 0.90*/)
								{
									m_hCaptureCompleteThreadHandle = ::CreateThread(NULL, 0, &StopCapThread, 0, 0, NULL);
									IsGetQtyInCap = false;
...
int MARVIS_Auth_InitDevice(int fd, const char *pcProductName, int ProductNameLen, MARVIS_DEVICE_INFO *StDeviceInfo)
{
	...
	try
	{
		...
		int retValue = MSSD_CheckLoadModel(modelBuffer, modelBufferLen);
