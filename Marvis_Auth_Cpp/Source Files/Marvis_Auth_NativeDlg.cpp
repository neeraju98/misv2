...
void CCommTesterDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_CAPTURE_TYPE_VALUE, Capture_Type_Value);
}
...
void CCommTesterDlg::OnStartCaptureButtonClicked()
{
	...
	if (m_strTimeOut1 == 0 || m_strTimeOut1 > 15000)
		m_strTimeOut1 = 15000;
	if (Capture_Type_Value<0 || Capture_Type_Value>3)
		Capture_Type_Value = 0;
	int returnValue = MARVIS_Auth_StartCapture(m_strQty1, m_strTimeOut1, m_CapturePreviewCallBack, m_CaptureCompleteCallBack, empID, Capture_Type_Value);
...
void obCaptureCompleteCallBack(int ErrorCode, MARVIS_IMAGE_QUALITY *QtyInfo, MARVIS_IMAGE_DATA *m_previewImageData)
{
	if (MARVIS_AUTH_E_SUCCESS == ErrorCode)
	{
		if (g_ImagePreviewFrame)
		{
			...
			if (g_ImagePreviewFrame->pCapturedImage)
			{
				...
				_snprintf_s(szImageQuality, sizeof(szImageQuality), sizeof(szImageQuality), "QLTY : [ %d] Live:[ %d] score-0:[ %f]  score-1:[ %f] ",
					QtyInfo->Quality, QtyInfo->percent_occlusion, QtyInfo->sclera_mean, QtyInfo->sharpness);
	...
	else
	{
		...
		if (MARVIS_AUTH_E_FAKE_IRISFOUND == ErrorCode)
		{
			_snprintf_s(szImageQuality, sizeof(szImageQuality), sizeof(szImageQuality), "QLTY : [ %d] ",
				QtyInfo->Quality);
			...
			strExt = _T("output\\") + gpCommTesterDlg->m_EmpId + _T("_UnStable_") + cTimeStr + _T(".bmp");

			FILE* pEncodedFile = NULL;
			fopen_s(&pEncodedFile, CT2A(strExt), "wb");
			fwrite(m_previewImageData->m_BitmapImage, 1, m_previewImageData->m_BitmapImageLen, pEncodedFile);
			fclose(pEncodedFile);
		}
			

		else if (MARVIS_AUTH_MODEL_FAKE == ErrorCode)
		{
			_snprintf_s(szImageQuality, sizeof(szImageQuality), sizeof(szImageQuality), "QLTY : [ %d] Live:[ %d] score-0:[ %f]  score-1:[ %f] ",
				QtyInfo->Quality, QtyInfo->percent_occlusion, QtyInfo->sclera_mean, QtyInfo->sharpness);
			...
			strExt = _T("output\\") + gpCommTesterDlg->m_EmpId + _T("_ModelFake_") + cTimeStr + _T(".bmp");
			...
			if (gpCommTesterDlg)
			{
				if (nImageLengthWithBitmapHeader > 0 && strlen((char *)pBitmapImage) > 0)
				{
					...
					_snprintf_s(szImageQuality, sizeof(szImageQuality), sizeof(szImageQuality), 
						...
						/*pImagePreviewParams->imgQtyInfo.sclera_mean, pImagePreviewParams->imgQtyInfo.sharpness*//*, pImagePreviewParams->imgQtyInfo.iris_diameter,
