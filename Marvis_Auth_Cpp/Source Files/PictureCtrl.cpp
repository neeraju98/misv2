...
void CPictureCtrl::setFaceRect(IRIS_COORDINATES *rData, int NoOfIris)
{
	NoOfIrisCnt = NoOfIris;
	memset(m_pIrisCoordinates, 0, sizeof(m_pIrisCoordinates));
	for (int i = 0; i < NoOfIrisCnt; i++)
	{
		if (rData[i].x == 0 && rData[i].x == 0)
			continue;
		m_pIrisCoordinates[i] = rData[i];
	}
...
void CPictureCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if(m_bIsPicLoaded)
	{
		...
		::Graphics graphics_(&image);

		if (m_bIsIrisDraw == TRUE)
		{
			...
			for (int i = 0; i < NoOfIrisCnt; i++)
			{
				if (m_pIrisCoordinates[i].x == 0 && m_pIrisCoordinates[i].y == 0)
					continue;

				_itow(m_pIrisCoordinates[i].r, string, 10);

				float d = m_pIrisCoordinates[i].r * 1;
				float X = m_pIrisCoordinates[i].x - d;
				float Y = m_pIrisCoordinates[i].y - d;
				float R = d * 2;

				// Create rectangle for ellipse.
				RectF rectF(X, Y, R, R);
				graphics_.DrawEllipse(&blackPen, rectF);
			}
			m_bIsIrisDraw = FALSE;
		}
